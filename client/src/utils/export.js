...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .invoice-table td:nth-child(3), .invoice-table td:nth-child(5) { text-align: right; }
        .invoice-table td:last-child { font-weight: 600; text-align: right; }
...
<th style="width:15%; text-align: right;">PRICE (${curSymbol})</th>
<th style="width:10%; text-align: center;">QTY</th>
<th style="width:20%; text-align: right;">TOTAL (${curSymbol})</th>
</tr>
</thead>
<tbody>
${rows}
...
export async function downloadInvoice(invoice, bankDetails) {
  ...
  const safeClient = (invoice.billedTo || 'Client').replace(/[/\\?%*:|"<>]/g, '-');
  const filename = `Invoice-${safeClient}-${invoice.date}.pdf`;
...
// ─── LADINA FREIGHT SOLUTIONS LIMITED ────────────────────────────────────────

export function generateLFInvoiceHTML(invoice, bankOptions = [], options = {}) {
  const { invoiceNo, billedTo, date, currency, lineItems, total } = invoice;
  const curSymbol = currency || 'USD';
  ...
  const categories = [...new Set(lineItems.map(item => item.category).filter(c => c && c.toLowerCase() !== 'other'))];
  const serviceTypeHTML = categories.length > 0
    ? `<p style="margin-top: 8px;"><span class="label">SERVICE TYPE:</span> ${categories.join(' / ').toUpperCase()}</p>`
    : '';
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media print {
          ...
          .invoice-container { width: 100%; min-height: auto; padding: 0; margin: 0; }
          tr { page-break-inside: avoid; }
          .invoice-table thead { display: table-header-group; }
        }
        .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; border-bottom: 2px solid #2E7D32; padding-bottom: 15px; }
...
<div style="page-break-inside: avoid;">
  ...
  <p class="thank-you">Thank you for your business!</p>
  <p class="terms">Payment is required within 14 business days of invoice date.</p>
  <p>Please send remittance confirmation to ladinafreightssolutionsltd@gmail.com</p>
  </div>
</div>
...
export function previewLFInvoice(invoice, bankDetails) {
  return generateLFInvoiceHTML(invoice, bankDetails);
}

export async function downloadLFInvoice(invoice, bankDetails) {
  let html = generateLFInvoiceHTML(invoice, bankDetails);
  const safeClient = (invoice.billedTo || 'Client').replace(/[/\\?%*:|"<>]/g, '-');
  const filename = `LF-Invoice-${safeClient}-${invoice.date}.pdf`;

  try {
    const logoRes = await fetch('/llogo.jpeg');
    if (logoRes.ok) {
      const blob = await logoRes.blob();
      const base64 = await new Promise((resolve) => {
        const reader = new FileReader();
      ...
      html = html.replace('src="/llogo.jpeg"', `src="${base64}"`);
    }
  } catch { /* logo optional */ }
