...
import { api } from '../api';
import { previewLFInvoice } from '../utils/export';

function uid() {
  return `id_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
}

export default function LFInvoiceEditor({ invoiceId, onClose, onSaved, onPreview }) {
    ...
    <div className="modal-overlay" style={{
      ...
      <section className="card" style={{
        ...
        <header style={{
          ...
          <div>
            <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800, color: 'var(--brand)' }}>{isNew ? 'New LF Invoice' : 'Edit LF Invoice'}</h2>
            <p className="muted" style={{ fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              {invoiceNo || 'Draft'} · Ladina Freight Solutions Limited
              ...
              <div className="card" style={{ background: '#fcfcfc', border: '1px dashed #e2e8f0', padding: '1.5rem', marginBottom: '1.5rem' }}>
                <div className="grid-2" style={{ gap: '1rem' }}>
                  <div style={{ gridColumn: 'span 2' }}>
                    ...
                    <input value={itemName} onChange={(e) => setItemName(e.target.value)} placeholder="e.g. Sea Freight - Mombasa to Rotterdam" />
                  </div>
                  <div style={{ gridColumn: 'span 2' }}>
                    <label>Description / Details</label>
                    <input value={itemDescription} onChange={(e) => setItemDescription(e.target.value)} placeholder="e.g. 20ft container, door-to-port" />
                  </div>
          ...
          <div className="btn-row" style={{ marginTop: '3rem', justifyContent: 'flex-end', gap: '1rem' }}>
            <button
              ...
              onClick={() => {
                ...
                if (onPreview) {
                  onPreview(previewLFInvoice({ invoiceNo, billedTo, date, currency, lineItems, total }, selectedAccount));
                }
              }}
