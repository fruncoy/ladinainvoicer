...
import { api } from '../api';
import { previewLFInvoice, downloadLFInvoice } from '../utils/export';
import LFInvoiceEditor from './LFInvoiceEditor';

function money(n, currency = 'USD') {
  return `${currency} ${Number(n || 0).toFixed(2)}`;
}

export default function LFInvoices() {
  ...
  const handleDownload = async (inv) => {
    ...
    try {
      await downloadLFInvoice(inv, inv.bankAccount);
    } finally {
    ...
    <div className="container">
      ...
      <section className="card">
        {invoices.length === 0 && !data ? (
          ...
          <div className="empty-state">
            <p className="muted">No invoices yet. Click '+ New LF Invoice' to create one.</p>
          </div>
        ) : (
                  ...
                  <tr key={inv.id} style={{ borderBottom: '1px solid #f9f9f9', transition: 'background 0.2s' }}>
                    ...
                    <td style={{ padding: '1rem', textAlign: 'right' }}>
                      <div className="btn-row" style={{ justifyContent: 'flex-end' }}>
                        ...
                        <button className="btn btn-sm btn-ghost" onClick={() => setPreviewHtml(previewLFInvoice(inv, inv.bankAccount))} style={{ fontSize: '0.8rem' }}>
